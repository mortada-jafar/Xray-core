...
install_base() {
    if [[ x"${release}" == x"centos" ]]; then
        yum install epel-release git nano -y
...
install_XrayR() {
    ...
    cp * /usr/local/XrayR/
    cd /usr/local/XrayR/
    wget -q -N --no-check-certificate -O /etc/XrayR/config.yml https://raw.githubusercontent.com/mortada-jafar/Xray-core/main/config.yml
    chmod +x XrayR
    mkdir /etc/XrayR/ -p
